...
## 🛡️ Антивирусы ругаются на exe — это нормально?

Некоторые антивирусы могут помечать `Freehop.exe` как подозрительный. Это ложное срабатывание: программа собрана через PyInstaller, запрашивает права администратора и изменяет сетевые параметры в реестре — по этим признакам эвристика антивирусов часто путает такие утилиты с malware, даже когда код полностью безобиден.

Результат проверки на VirusTotal: [посмотреть отчёт](https://www.virustotal.com/gui/file/5df19c855eb41baccd454bf822b9254717956d67bf83dbd84963b340436675df/details)

Если не доверяешь готовому exe — исходный код открыт, можно собрать самому (см. раздел «Установка» выше).
